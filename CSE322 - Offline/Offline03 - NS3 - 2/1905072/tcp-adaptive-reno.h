...
#ifndef TCPADAPTTIVERENO_H
#define TCPADAPTTIVERENO_H
...
class TcpAdaptiveReno : public TcpWestwoodPlus
{
  ...
  protected:
    ...
    double EstimateCongestionLevel(const Time& rtt);
  ...
  private:
    ...
    Time m_lastRtt{Time::Max()}; //!< Last RTT observed. It's equal to RTT of j-th packet loss when GetSSThresh
                    //!< is called.
    Time m_congRtt{Time::Max()}; //!< Estimated congestion RTT

    double m_smoothingFactor; // !< Smoothing factor for calculating m_congRtt
};

} // namespace ns3

#endif // TCPADAPTTIVERENO_H
